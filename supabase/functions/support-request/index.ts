...
const SUPPORT_EMAIL = "support@budgetapp.site";
...
async function sendEmail(to: string[], subject: string, html: string, replyTo?: string) {
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "BudgetApp Support <noreply@budgetapp.site>",
      to,
      subject,
      html,
      reply_to: replyTo,
...
serve(async (req) => {
  ...
  try {
    ...
    if (subject.length > 100 || message.length > 2000) {
      return new Response(
        JSON.stringify({ error: 'Konu veya mesaj çok uzun' }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
...
<head>
  ...
  <style>
    ...
    .message-box { background: #fef2f2; border-left: 4px solid #ef4444; padding: 20px; border-radius: 4px; margin-top: 20px; }
    ...
    .reply-btn { display: inline-block; background: #ef4444; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; margin-top: 20px; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>Bu e-posta BudgetApp destek formundan otomatik olarak gönderilmiştir.</p>
    </div>
  </div>
</body>
</html>
    ...
    // Send email to support address with reply-to set to user's email
    const emailResponse = await sendEmail(
      [SUPPORT_EMAIL],
      `🆘 Destek Talebi: ${subject}`,
      emailHtml,
      email
    );

    console.log(`Support request email sent to ${SUPPORT_EMAIL}:`, emailResponse);

    return new Response(
      JSON.stringify({ success: true, message: 'Destek talebiniz gönderildi' }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );

  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    console.error("Error in support-request function:", error);
    return new Response(
